# Context and Response Compaction — Python
...
- src/config.py:18 — export interface FooConfig
- src/server.py:42 — function createServer(config: FooConfig)
...
...
## Confirmed evidence
- E01 `src/auth.py:31-55` — token parsed here.
- E02 `src/user.py:80-103` — user lookup occurs here.
...
## Evidence
- `path/file.py:10-32` — compact evidence statement
- ...
...
Implement deterministic compaction primarily as pure Python functions/classes.
...
Prefer typed inputs/outputs without global state.

The compactor may use a conservative local token estimate only for deciding when to compact before another API call.

Provider-reported DeepSeek usage remains authoritative for billing/token reporting.
