# Security, Testing, and Acceptance — Python
...
- use `asyncio.create_subprocess_exec()` or another fixed-argument Python subprocess API,
...
Before completion run the actual Python project commands, preferably:
...
Also run an MCP in-memory smoke test and a stdio startup smoke test.

Use actual package scripts and document them in README.

## 11. Acceptance checklist

- [ ] DeepSeek_MCP implementation is Python.
- [ ] `pyproject.toml` defines the project and console entrypoint.
- [ ] Official MCP Python SDK is used.
- [ ] Automated tests use pytest.
- [ ] ruff checks/format checks pass.
- [ ] mypy passes for the configured source scope.
