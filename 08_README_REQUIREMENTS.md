# README Requirements — Python DeepSeek_MCP
...
Document exact supported versions based on the finished `pyproject.toml`.

At minimum:

- Python 3.11+ for DeepSeek_MCP
- `uv` as the recommended Python environment/package manager
- Claude Code
- DeepSeek API key

If Claude Code's own installation has additional prerequisites, document them separately from the DeepSeek_MCP Python runtime.
...
If `.env` loading is actually implemented, document the real optional step. Do not add dotenv only for documentation convenience.
...
List the exact real Python commands. They should resemble:
...
Also document the actual in-memory MCP test and stdio smoke-test command.
