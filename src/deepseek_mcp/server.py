...
def create_app(
    ...
    *,
    ...
    @mcp.tool(
        description=(
            "Delegate a high-context, read-heavy codebase task to the DeepSeek "
            "worker (repository exploration, architecture tracing, search, "
            "summarization, evidence collection). Output is advisory and ends "
            "with a DeepSeek token usage footer."
        )
    )
